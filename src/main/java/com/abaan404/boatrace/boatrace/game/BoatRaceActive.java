...
public class BoatRaceActive {
    private final BoatRaceConfig config;

    public final GameSpace gameSpace;
    private final BoatRaceMap gameMap;

    private final Object2ObjectMap<PlayerRef, BoatRacePlayer> participants;
    private final BoatRaceSpawnLogic spawnLogic;
    private final BoatRaceStageManager stageManager;
    private final boolean ignoreWinState;
    private final BoatRaceTimerBar timerBar;
    private final ServerWorld world;

    private BoatRaceActive(GameSpace gameSpace, ServerWorld world, BoatRaceMap map, GlobalWidgets widgets, BoatRaceConfig config, Set<PlayerRef> participants) {
        ...
        for (PlayerRef player : participants) {
            this.participants.put(player, new BoatRacePlayer());
        }

        this.stageManager = new BoatRaceStageManager();
        this.ignoreWinState = this.participants.size() <= 1;
        this.timerBar = new BoatRaceTimerBar(widgets);
    }

    public static void open(GameSpace gameSpace, ServerWorld world, BoatRaceMap map, BoatRaceConfig config) {
        gameSpace.setActivity(game -> {
            Set<PlayerRef> participants = gameSpace.getPlayers().participants().stream()
                    .map(PlayerRef::of)
                    .collect(Collectors.toSet());
            GlobalWidgets widgets = GlobalWidgets.addTo(game);
            BoatRaceActive active = new BoatRaceActive(gameSpace, world, map, widgets, config, participants);
    ...
    private void tick() {
        ...
        BoatRaceStageManager.IdleTickResult result = this.stageManager.tick(time, gameSpace);
