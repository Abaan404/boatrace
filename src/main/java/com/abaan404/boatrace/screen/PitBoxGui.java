...
public class PitBoxGui extends SimpleGui {
    ...
    private void setState(State state) {
        ...
        if (state == State.READY) {
            player.playSoundToPlayer(SoundEvents.BLOCK_NOTE_BLOCK_BIT.value(), SoundCategory.RECORDS, 1.0f,
                    NoteBlock.getNotePitch(12));
        }

        if (state == State.FAIL) {
            player.playSoundToPlayer(SoundEvents.BLOCK_NOTE_BLOCK_DIDGERIDOO.value(), SoundCategory.RECORDS, 1.0f,
                    NoteBlock.getNotePitch(12));
        }
    ...
    public void onTick() {
        ...
        Countdown.TickResult result = this.countdown.tick(this.player.getServerWorld());
